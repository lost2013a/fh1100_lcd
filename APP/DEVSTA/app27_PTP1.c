...
static void Showsta(WM_MESSAGE * pMsg)
{
	...
	//offset1=T4_OFFSET;
	ptp1_sta_signal				=		(DEV_STA) (CAN1_RX0_BUF[T4_OFFSET+2]&0x01);						//PTP1信号状态	
	ptp1_sta_synch				=		(DEV_STA)((CAN1_RX0_BUF[T4_OFFSET+2]&0x02)>>1);				//同步状态	
	ptp1_time_valid				=		(DEV_STA)((CAN1_RX0_BUF[T4_OFFSET+2]&0x04)>>2);			  //时间连续状态 
//	ptp1_time_valid				=		(DEV_STA)((CAN1_RX0_BUF[T4_OFFSET+5]&0x10)>>3);			//ptp1源时间连续状态
	ptp1_twostep					=		CAN1_RX0_BUF[T4_OFFSET+3];											//单步，双步
	ptp1_workmode					=		CAN1_RX0_BUF[T4_OFFSET+4];						          //工作模式
	ptp1_state				    =		CAN1_RX0_BUF[T4_OFFSET+5];						          //PTP状态
	
	ptp1_LS_come					=		(DEV_STA)((CAN1_RX0_BUF[T4_OFFSET+1]&0x80)>>7);		//闰秒预告
	ptp1_LS_sign					=		(DEV_STA)((CAN1_RX0_BUF[T4_OFFSET+1]&0x40)>>6);		//闰秒标志
...
void _cbCallback27(WM_MESSAGE * pMsg) 
{
  switch (pMsg->MsgId) {
    case WM_PAINT:
