...
NVIC_PENDSVSET  EQU     0x10000000                              ; 通过给中断控制寄存器(NVIC_INT_CTRL)的 bit28 写1 来触发软件中断
...
OSCtxSw
    LDR     R0, =NVIC_INT_CTRL          ;中断控制寄存器
    LDR     R1, =NVIC_PENDSVSET         ; 通过给中断控制寄存器(NVIC_INT_CTRL)的 bit28 写1 来触发软件中断 
    STR     R1, [R0]
    BX      LR
...
PendSV_Handler
    ...
    MRS     R0, PSP                               ;R0=PSP
    CBZ     R0, PendSVHandler_nosave              ;  如果第一次做任务切换 PSP 为 0 就转移到 PendSV_Handler_Nosave
    ...
    LDR     R1, =OSTCBCurPtr                                    ; R1=&OSTCBCur
