...
void  OSIntExit (void)
{
    ...
    if (OSRunning != OS_STATE_OS_RUNNING) {                 /* UCOS是否运行                                 */
        return;                                             /* No                                                     */
    }

    CPU_INT_DIS();																					//关中断
    if (OSIntNestingCtr == (OS_NESTING_CTR)0) {             /* 中断嵌套计数是否为 0，确保其不会等于负数。     */
        CPU_INT_EN();																				//开中断
        return;
    }
    OSIntNestingCtr--;																			//中断嵌套－1
    if (OSIntNestingCtr > (OS_NESTING_CTR)0) {              /*中断嵌套数>0，还有中断，不需要做任务切换直接退出         */
        CPU_INT_EN();                                       /* Yes                                                    */
        return;
    }

    if (OSSchedLockNestingCtr > (OS_NESTING_CTR)0) {        /* 检查调度器是否加锁，如果加锁的话就直接跳出     */
        CPU_INT_EN();                                       /* Yes                                                    */
        return;
    }
    ...
    OSIntCtxSw();                                           /*不需要像 OSCtxSW()一样先保存当前任务现场，只需要做 OSCtxSW()的后半部分工作，也就是从
																														将要执行的任务堆栈中恢复 CPU 寄存器的值。                    */
    CPU_INT_EN();
}
...
void  OSSched (void)
{
    ...
    OSTCBHighRdyPtr = OSRdyList[OSPrioHighRdy].HeadPtr;			//OSTCBHighRdyPtr 指向将要切换任务的 OS_TCB
    if (OSTCBHighRdyPtr == OSTCBCurPtr) {                  	//判断要运行的任务是否是正在运行的任务
        CPU_INT_EN();                                       //是就不切换，重开中断，退出                      */
        return;
    }
